...
function addItemsToTable(name, id, sex, speakingma, writingma, listeningma,
  ...
  if (speakingma == undefined) {
    ...
    td4.innerHTML = speakingma;
  }
  if (writingma == undefined) {
    td5.innerHTML = 0;
  } else {

    td5.innerHTML = writingma;
  }
  if (listeningma == undefined) {
    td6.innerHTML = 0;
  } else {

    td6.innerHTML = listeningma;
  }
  ...
  const sleep = async (milliseconds) => {
    ...
    //Sum all score
    let to1 = parseFloat(speakingma)
      + parseFloat(writingma)
      + parseFloat(listeningma)
      + parseFloat(readingma)
      + parseFloat(hApr);
    let sub = to1 / 5;
